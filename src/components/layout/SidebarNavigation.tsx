...
export function SidebarNavigation() {
  const { currentStepIndex, goToStep, responses } = useAudit();
    ...
    <div className="hidden lg:block w-72 shrink-0 space-y-4">
      <div className="sticky top-24">
        ...
        <nav className="space-y-1">
          {ISO_27001_DATA.map((section, index) => {
            const isCurrent = index === currentStepIndex;
            const domainQuestions = section.controls.flatMap(c => c.questions);
            const isCompleted = domainQuestions.length > 0 && 
                               domainQuestions.every(q => !!responses[q.id]?.status);
